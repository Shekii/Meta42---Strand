...
function manual() {
//RETRIEVE HASHTAGS FROM TWITTER 
//
	global $dbh;
	global $template;
	// $query = $dbh->prepare("
	// 						select accounts_queue.accountid, 
	// 						accounts.data2, 
	// 						accounts.data3 
	// 						from accounts_queue
	// 						join accounts on accounts_queue.accountid = accounts.id 
	// 						where accounts.companyid = ? and accounts.id = accounts_queue.accountid limit 1");
	...
	$twitter = new TwitterOAuth(TWITTER_APIKEY, 
								TWITTER_APISECRET/*, $data2, $data3*/);
	...
	$response = $twitter->oAuthRequest($path, 'GET', $params);

	$decoded = json_decode($response);

	$trends = $decoded[0]->trends;

	$template->set('trends', $trends);
...
function view_schedule() {
	...
	foreach ($schedule as $s) {
		...
		$query = $dbh->prepare("SELECT day FROM accounts_schedule WHERE schedule_id = ?");
		$query->execute(array($s[0]));
		$dateTime = $query->fetchAll();	

	}

	 $template->set('schedule',$schedule);
	 $template->set('days',$dateTime);
...
function scheduleupdate() {
	...
	$query = $dbh->prepare("insert into schedules (companyID, scheduleName) values (?,?)");
	$query->execute(array($_SESSION['user']['companyid'],$_POST['scheduleName']));
 
	$query = $dbh->prepare("select LAST_INSERT_ID() from schedules");
	$query->execute();
	$schID = $query->fetch(PDO::FETCH_NUM);
	$schID = $schID[0];
		...
		foreach ($_POST['days'] as $day) {
			if (!empty($_POST['time'])) {
				foreach ($_POST['time'] as $time) {
					...
					$query = $dbh->prepare("insert into accounts_schedule (companyid,accountid,userid,timezone,day,time,record_created,schedule_id) values (?,?,?,?,?,?,?,?)");
					$query->execute(array($_SESSION['user']['companyid'],$accountId,$_SESSION['user']['loggedin'],$_POST['timezone'],$day,$time,time(),$schID));
	...
	if (!empty($_POST['accounts'])) {
		foreach ($_POST['accounts'] as $account) {
			if (in_array($account,$present)) {
					...
					else if($_POST['when'] == 'specific'){
						$_POST['when'] = 2;
						$_SESSION['notification']['message'] = '<b>Yay!</b> Your update is queued and will be posted on the day you selected.';
					}

					if ($_POST['when'] == 2 ) {
						$time = strtotime($_POST['specificTime']);
						$date = strtotime($_POST['pickDate']);

						$midnight = strtotime("00:00");
						$s_time = $time - $midnight;

						$_POST['when'] = ($date + $s_time);

						$_SESSION['notification']['message'] = $_POST['when'];
